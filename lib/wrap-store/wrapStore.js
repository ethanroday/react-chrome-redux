...
exports.default = function (store, _ref) {
  var portName = _ref.portName,
      ...
      _ref$allowMessageConn = _ref.allowMessageConnections,
      allowMessageConnections = _ref$allowMessageConn === undefined ? false : _ref$allowMessageConn;
  ...
  */
  var connectState = function connectState(sendMessageFn, sendResponse) {

    var serializedMessagePoster = (0, _serialization.withSerializer)(serializer)(function () {
      return sendMessageFn.apply(undefined, arguments);
    });
    ...
    var initialStateSender = sendResponse || serializedMessagePoster;
    initialStateSender({
  ...
  var connectFromPort = function connectFromPort(port) {
    if (port.name !== portName) {
      return;
    }

    var unsubscribe = connectState(port.postMessage);
  ...
  if (allowMessageConnections) {
    var connectFromMessage = function connectFromMessage(message, sender, sendResponse) {
      if (message.type === _constants.CONNECT_TYPE) {
